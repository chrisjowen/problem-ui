...
<h1 class="mb-5 text-xl text-primary-600">Where</h1>
<div class=" mb-4">
  {#if stakeholder.regions}
    {#each stakeholder.regions as region}
      <span class="text-xs bg-primary-600 text-white p-2 mr-1 mb-1"> {region} </span>
    {/each}
  {:else}
...
<h1 class="text-xl mb-2 mt-5 text-primary-600">When</h1>
<div>
  <p>
    {stakeholder.when}
  </p>
